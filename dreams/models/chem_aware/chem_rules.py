"""
化学规则引擎 (Chemical Rule Engine) — 模块 B [v4 规则库扩展版]

核心改动（v3 → v4，任务一 P0 立即注入高可信规则）：
  1. 规则库从 127 条扩展至 ~335 条，并入三个高可信规则源：
     - CompMS2Miner 子结构库（Neut.loss → NL，frag → CF）
     - MS-FINDER 9 条氢重排规则（新建 HR 类别）
     - ESI(+) 常见官能团碎裂规则（教科书/经验）
  2. 规则数据外置到 chem_rules_v2.json（数据与代码分离，每条带 source 溯源）；
...
# 规则库 JSON 路径（与本文件同目录）
_RULES_JSON = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chem_rules_v2.json")
...
def _build_rule_list() -> List[ChemRule]:
    ...
    优先从同目录 chem_rules_v2.json 加载（任务一 P0 产出的扩展规则库，~335 条，
...
def _load_rules_from_json() -> List[ChemRule]:
    """从 chem_rules_v2.json 加载规则；失败返回空列表。"""
...
class ChemicalRuleEngine(nn.Module):
    ...
    def __init__(
        ...
        self._hr_rules: List[Tuple[int, ChemRule]] = []          # v4: 氢重排规则
        ...
        # hr_shift 类（v4 氢重排）：value 存带符号氢重排数 n_H
    ...
    def forward(
        ...
        # =================================================================
        # hr_shift 类规则（v4 氢重排，MS-FINDER 9 条 HR 规则）
