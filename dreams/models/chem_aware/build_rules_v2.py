"""
build_rules_v2.py — 任务一 (P0) 规则库构建脚本
================================================================================
将三个高可信规则源并入现有 127 条基线规则，去重后输出 chem_rules_v2.json：
...
流程（对应任务文档 1.4 执行步骤）：
  ...
  6. 合并输出 chem_rules_v2.json（每条带 source 溯源字段）

本脚本零第三方依赖（仅标准库 + ast），可在无 torch 环境运行。

用法:  python build_rules_v2.py
"""
...
OUT_JSON = HERE / "chem_rules_v2.json"
...
def main():
    print("=" * 72)
    print("任务一 (P0)：构建 chem_rules_v2.json")
    print("=" * 72)
